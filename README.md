...
- **サブチャンネル非表示**: スレッドなどの子チャンネルは一覧に出さず、親チャンネル選択でまとめて扱う
- **DM対応**: 友達とのダイレクトメッセージをリスト表示し、選択して同期可能
...
# CLI モードで過去同期の進捗を定期表示
python main.py --cli --ui-kakodouki
```

### CLI モード

- `--cli` は、過去同期とリアルタイム保存だけを行う軽量モードです。
- 起動時に取得可能なサーバー内チャンネルと DM を順番に同期し、その後はメッセージ受信の保存だけを続けます。
- `--ui-kakodouki` を付けると、過去同期の進捗を定期的に表示します。
...
2. **Direct Messages パネル（中）**
   - 友達とのダイレクトメッセージ一覧
   - 💬 アイコンで表示
   - 各 DM の保存済みメッセージ数を表示
   - DM をクリックするとメッセージを閲覧・同期できます

3. **Channels パネル（下）**
   - 選択されたサーバー内の親チャンネル一覧
   - 各チャンネルの保存済みメッセージ数を表示
   - チャンネルをクリックすると、その配下のスレッドもまとめて閲覧・同期できます
...
├──────────────────────────────────────┤
│ Server │ DM    │ Sync Status         │
│ #guild │💬User ├──────────────────────┤
│ #guild │💬User │ Message Viewer       │
│        │       │                      │
│ ───── │ ─────┤                      │
│ #ch   │       │                      │
│ #gen  │       │                      │
...
- `guild_id`: Discord サーバーID（DM の場合は NULL）
- `channel_name`: チャンネル名
- `parent_channel_id`: 親チャンネルID（スレッド等）
- `channel_type`: チャンネル種別（text/voice/forum/thread/dm など）
